...
export function buildRook(group, mat) {
    ...
    // BASE: Cylinder (r=0.26, h=0.06) at y=0.03
    add(new THREE.CylinderGeometry(0.26, 0.28, 0.06, 24), 0.03);
    // UPPER BASE: Cylinder (r=0.20, h=0.24) at y=0.36
    add(new THREE.CylinderGeometry(0.18, 0.22, 0.5, 24), 0.3);
    // RING: Cylinder (r=0.24, h=0.05) at y=0.70
    add(new THREE.CylinderGeometry(0.24, 0.24, 0.05, 24), 0.55);
    // PLATFORM: Cylinder (r=0.26, h=0.06) at y=0.755
    const plat = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.26, 0.06, 24), mat);
    plat.position.y = 0.6;
    group.add(plat);
    // CRENELLATIONS: 8 boxes (0.06x0.10x0.06) at radius 0.20, y=0.845
    // (Spaced at 45° intervals around the platform edge)
    for (let i = 0; i < 8; i++) {
        ...
        const b = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.14, 24), mat);
