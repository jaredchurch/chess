...
fn test_en_passant() {
    ...
    assert!(
        board.get_piece_at(Square::F5).is_none(),
        "Captured pawn at F5 should be removed"
    );
    assert_eq!(
        board.get_piece_at(Square::F6).unwrap().piece_type,
        PieceType::Pawn
    );
}
...
fn test_promotion() {
    ...
    let queen_promo = promotions
        .iter()
        .find(|m| m.flag == MoveFlag::Promotion(PieceType::Queen))
        .unwrap();
    board.make_move(**queen_promo);
    assert_eq!(
        board.get_piece_at(Square::E8).unwrap().piece_type,
        PieceType::Queen
    );
}
