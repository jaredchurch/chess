...
pub fn generate_king_moves(board: &Board, square: Square, moves: &mut Vec<Move>) {
    ...
    if color == Color::White && square == Square::E1 {
        ...
        if (board.castling_rights & 0x1) != 0
            && !board.occupancy[2].get(Square::F1.as_u32())
            && !board.occupancy[2].get(Square::G1.as_u32())
        ...
        if (board.castling_rights & 0x2) != 0
            && !board.occupancy[2].get(Square::D1.as_u32())
            && !board.occupancy[2].get(Square::C1.as_u32())
            && !board.occupancy[2].get(Square::B1.as_u32())
            && !crate::move_gen::is_in_check(board, Color::White)
        ...
        if (board.castling_rights & 0x4) != 0
            && !board.occupancy[2].get(Square::F8.as_u32())
            && !board.occupancy[2].get(Square::G8.as_u32())
        ...
        if (board.castling_rights & 0x8) != 0
            && !board.occupancy[2].get(Square::D8.as_u32())
            && !board.occupancy[2].get(Square::C8.as_u32())
            && !board.occupancy[2].get(Square::B8.as_u32())
            && !crate::move_gen::is_in_check(board, Color::Black)
...
pub fn get_king_attacks(square: Square) -> Bitboard {
    super::lookup::get_lookup_tables().king_attacks[square as usize]
}

#[allow(dead_code)]
pub fn get_king_attacks_slow(square: Square) -> Bitboard {
